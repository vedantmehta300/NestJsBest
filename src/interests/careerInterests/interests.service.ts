...
import InterestsEntity from '../../models/Interests/careerInterest.entity';
...
export class InterestsService {
  ...
  async getByParentId(parentId: string, { size, skip }: any): Promise<any> {
    const parentInterest = await this.interestsRepository.findOne({
      id: parentId,
    });
    const _take = size || 100;
    const _skip = skip || 0;
    const [result, total] = await this.interestsRepository.findAndCount({
      // parent: parentInterest,
      where: { parent: parentInterest },
      take: _take,
      skip: _skip,
    });
